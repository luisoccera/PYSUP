...
export function HomeScreen({ name, countryName, stats, items, wide, onDiscover, onFriends, onOpen }: { name: string; countryName: string; stats: { watched: number; reviews: number; friends: number; saved: number }; items: ContentItem[]; wide: boolean; onDiscover: () => void; onFriends: () => void; onOpen: (item: ContentItem) => void }) {
  ...
  const pageInsets = { paddingHorizontal: responsive.gutter, paddingTop: responsive.contentTop };
  const featured = items[0];
  if (!featured) return (
    <ScrollView style={styles.screen} contentContainerStyle={[styles.screenContent, responsive.isPhone && styles.screenContentPhone, responsive.isTablet && styles.screenContentTablet, pageInsets]}>
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={[styles.screenContent, responsive.isPhone && styles.screenContentPhone, responsive.isTablet && styles.screenContentTablet, pageInsets]} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.homeHero, wide && styles.homeHeroWide, responsive.isPhone && styles.homeHeroPhone]}>
        <ImageBackground source={featured.image} style={styles.homeHeroImage} imageStyle={styles.homeHeroRadius}>
          <LinearGradient colors={wide ? ['rgba(7,10,18,0.97)', 'rgba(7,10,18,0.62)', 'rgba(7,10,18,0.1)'] : ['rgba(7,10,18,0.18)', 'rgba(7,10,18,0.98)']} start={{ x: 0, y: 0 }} end={{ x: wide ? 1 : 0, y: 1 }} style={[styles.homeHeroGradient, responsive.isPhone && styles.homeHeroGradientPhone]}>
            <View style={styles.heroInfo}>
              ...
              <Text style={[styles.heroTitle, responsive.isPhone && styles.heroTitlePhone]}>{featured.title}</Text>
              <Text style={styles.heroSubtitle}>{featured.subtitle}</Text>
              <View style={[styles.heroMeta, responsive.isPhone && styles.heroMetaPhone]}><Text style={styles.heroMetaText}>{featured.year || '—'}</Text><Text style={styles.heroMetaText}>{featured.duration}</Text><Text style={styles.heroMetaText}>{featured.maturity}</Text>{featured.providers[0] && <ProviderBadge id={featured.providers[0]} compact />}</View>
              <Text numberOfLines={3} style={styles.heroSynopsis}>{featured.synopsis}</Text>
              <View style={styles.heroActions}><Button label="Ver detalles" icon="play" onPress={() => onOpen(featured)} style={[styles.heroPrimary, responsive.isPhone && styles.heroActionPhone]} /><Button label="Descubrir más" icon="compass" variant="secondary" onPress={onDiscover} style={[styles.heroSecondary, responsive.isPhone && styles.heroActionPhone]} /></View>
            </View>
          </LinearGradient>
        </ImageBackground>
      </View>
      ...
      <View style={[styles.insightCard, responsive.isPhone && styles.insightCardPhone]}>
        ...
        <View style={[styles.insightStats, responsive.isPhone && styles.insightStatsPhone]}><Stat value={`${stats.watched}`} label="vistas" /><View style={styles.statDivider} /><Stat value={`${stats.saved}`} label="guardadas" /><View style={styles.statDivider} /><Stat value={`${stats.reviews}`} label="reseñas" /></View>
      </View>
    </ScrollView>
  );
}
